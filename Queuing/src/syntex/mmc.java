 ...
 * @author Hp
 */
public class mmc extends javax.swing.JInternalFrame {
    ...
    private void jPanel14MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jPanel14MouseClicked
      if(valid(t1.getText())&&valid(t2.getText())&&valid(t3.getText())){
        ...
        if(rw < 1){
          p00 += ((c * Math.pow(r, c))/(fact(c)*(c-r)));
        }
        else if(rw >= 1) {
              p00 += ((Math.pow(r, c))/(fact(c))) * ((c*u)/((c*u)-h));     
        }
        p0 = 1/p00;
        lq = p0*(((Math.pow(r, c+1)/c)/(fact(c)*Math.pow((1-r)/c,2))));
        wq = lq * x ;
        w = wq + y ;
        l = lq + r ;
