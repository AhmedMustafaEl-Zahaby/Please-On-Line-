 ...
 * @author Hp
 */
public class mmck extends javax.swing.JInternalFrame {

    /**
     * Creates new form mmck
     */
    public mmck() {
        initComponents();
    ...
    if(valid(t1.getText())&&valid(t2.getText())&&valid(t3.getText())&&valid(t4.getText())){
       x = tst(t1.getText());
       y= tst(t2.getText());
       c = tst(t3.getText());
       k = tst(t4.getText());
       double l , lq ,w , wq , h , u , r, rw ,p0;
       double p00 = 0;
       h = 1 / x ; 
       u = 1 / y ;
       r = h/u;
       ...
       for (double i = 0 ; i<= (c-1) ; i++){
              p00 += ((Math.pow(r,i)/(fact(i))));
          }
       if(rw == 1){
           p00 += ((Math.pow(r,c)/fact(c)) * ( k - c + 1));
       }
       else {
         p00 += ((Math.pow(r,c)/fact(c)) * ((1-Math.pow(rw , k - c + 1))/(1-rw)));  
       }
       p0 = 1 / p00 ;
       double pk = 0 ;
       pk =((Math.pow(h,k)/(((Math.pow(c,k - c) * fact(c) *(Math.pow(u,k))))))) * p0 ;
       double ld = (h*(1-pk));
       lq = ((rw * Math.pow(r,c) * p0)/(fact(c) * Math.pow(1-rw , 2))) * (1 - Math.pow(rw,k-c+1) - ((1-rw)*(k-c+1)*Math.pow(rw,k-c)));
       double pp = 0;
       for (double i = 0 ; i<= (c-1) ; i++){
              pp += ((c-i) * ((Math.pow(r,i)/(fact(i)))));
          }
       l = lq + c - (p0*pp);
       w = l/ld ;
       wq = lq / ld ;
